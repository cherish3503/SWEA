...
public class Swea4012chef {
	...
	private static int cook(int[][] S, int bit) {
		if(Integer.bitCount(bit) ==1) return 0;
...
public class Swea4012chef {
	...
	private static int choose(int[][] S, int[] dp, int depth, int bit) {
		int N = S.length;
		int cnt = bitCnt(bit);
		...
		return Math.min(choose(S, dp, depth+1, bit), choose(S, dp, depth+1, bit|(1<<depth)));
	}
